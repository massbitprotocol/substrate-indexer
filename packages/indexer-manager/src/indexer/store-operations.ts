...
import {Entity} from '@massbit/types';
    ...
    if (operation.operation === OperationType.Remove) {
      ...
      for (const field of operationModel.fields) {
        ...
        if (fieldValue !== undefined && fieldValue !== null) {
          switch (field.type) {
            case 'Date':
              dataBufferArray.push(Buffer.from(numberToU8a(fieldValue.getTime())));
              break;
            case 'BigInt':
            ...
            case 'Int':
              ...
              break;
            case 'Boolean':
              dataBufferArray.push(Buffer.from(numberToU8a(fieldValue ? 1 : 0)));
              break;
            case 'String':
              dataBufferArray.push(Buffer.from(fieldValue.toString()));
              break;
